...
class AuthService {
  ...
  Future<UserModel> login(String email, String password) async {
    ...
    if (response.statusCode == 200) {
      // 1. التحقق من نجاح العملية بوجود التوكين في الاستجابة الجذرية
      if (responseData.containsKey('user_token') &&
          responseData['user_token'] != null) {
        SharedPreferences prefs = await SharedPreferences.getInstance();

        // 2. حفظ التوكين
        await prefs.setString('user_token', responseData['user_token']);
        print("تم حفظ التوكين بنجاح");

        // 3. حفظ الدور (roles_name) إن وجد
        if (responseData.containsKey('roles_name') &&
            responseData['roles_name'] != null) {
          await prefs.setString('roles_name', responseData['roles_name']);
          print("تم حفظ الدور: ${responseData['roles_name']}");
        }

        return UserModel.fromJson(responseData);
      } else {
        // إذا لم يوجد التوكين، فهذا يعني أن الدخول فشل (مثلاً الإيميل خطأ)
        // نقوم بجلب رسالة الخطأ من السيرفر (msg)
        throw Exception(
          responseData['msg'] ?? 'الإيميل أو كلمة المرور غير صحيحة',
        );
      }
    } else {
  ...
  Future<Map<String, dynamic>> register({
    ...
    if (specialization != null)
      body['specialization'] = specialization; // أضيفي هذا السطر
    final response = await http.post(
  ...
  Future<List<dynamic>> getUpcomingAppointments() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'فشل جلب المواعيد: ${response.statusCode} - ${response.body}',
      );
    }
  }
  ...
  Future<List<VisitModel>> getVisits() async {
    ...
    final response = await http.get(
      Uri.parse(
        '${baseUrl}pat_visits',
      ), // تأكدي أن هذا هو الرابط الصحيح في api.php
      headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'},
    );
