...
Future<void> fetchVisits() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // 1. التأكد أننا ما زلنا في الصفحة قبل تحديث الحالة
      if (!mounted) return; 

      setState(() {
        // 2. استخدام المفتاح الصحيح كما ظهر في الـ Log: upcoming_Appointment
        if (data['status'] == 'success' && data.containsKey('upcoming_Appointment')) {
          visits = data['upcoming_Appointment'];
        } else {
          visits = []; // في حال عدم وجود بيانات
        }
        isLoading = false;
      });
    } else {
  ...
  Widget _buildVisitCard(Map visit) {
    String statusText = visit['status'] ?? "Pending"; 
